  ...
  async (req, res) => {
    try {
      ...
      if (curr > end_Date) {
        ...
        const auction_check = await query(
          "SELECT auction_status FROM auctions WHERE auction_status= 'closed' "
        );
        if (auction_check) {
...
router.get(
  ...
  async (req, res) => {
    ...
    auctionsOfSeller.map((auction) => {
      ...
      auction.image_url =
        "http://" + req.hostname + ":4000/" + auction.image_url;
    });
    res.status(200).json(auctionsOfSeller);
  }
);
...
router.get("", authorized, VerifyJWT, async (req, res) => {
  const query = util.promisify(conn.query).bind(conn);
  let search = "";
  if (req.query.search) {
